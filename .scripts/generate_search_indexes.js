...
const makeRacesSearchIndex = async () => {
	const locales = sync('./locales/*');
	...
	for (const locale of locales) {
		...
		const races = await readFileAsync(join(locale, 'races.json'));
		const index = elasticlunr(function () {
			if (localeName !== 'en') {
				this.use(elasticlunr[localeName]);
			}
		...
		const racesJson = JSON.parse(races);
		...
		writeFileAsync(
			join(locale, 'search.races.index.gen.json'),
			JSON.stringify(index.toJSON()),
		);
	}
}
